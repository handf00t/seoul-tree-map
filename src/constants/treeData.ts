// src/constants/treeData.ts
// 나무 관련 공통 상수 및 유틸리티

import { SpeciesInfo, SizeCategory, TreeType, TreeData } from '../types';

/**
 * 서울시 주요 수종 목록
 * 이름과 색상 정보를 포함
 */
export const availableSpecies: SpeciesInfo[] = [
 ...
 * 나무 타입별 색상 (fallback)
 */
export const treeTypeColors: Record<TreeType, string> = {
 ...
 */
export const getTreeColor = (tree: Partial<TreeData>): string => {
  // 1순위: 수종별 색상
 ...
 * 나무 타입 한글명 반환
 */
export const getTreeTypeName = (type: string): string => {
 ...
 * 크기 카테고리에 해당하는지 확인
 */
export const isInSizeCategory = (dbh: number, categoryId: string): boolean => {
 ...
 * DBH 값으로 크기 카테고리 찾기
 */
export const getSizeCategory = (dbh: number): SizeCategory | undefined => {
